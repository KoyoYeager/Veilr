...
#### D. デスピル後処理（Spill Suppression）

**概要**: 消去済みピクセルの隣接に残るターゲット色のにじみ（色カスト）を除去する後処理。

**なぜ効果的か**: クロマキー業界で「最後の品質仕上げ」として確立された手法。消去境界に残る微妙な色味を除去する。現在の残像の主要因。

**実装方針**:
```
消去後パスで、非マークピクセルの各チャネルを調整:
- ターゲット色がRGB(200,70,50)の場合
- 隣接ピクセルのR成分が不自然に高ければ、R = min(R, max(G,B) * 補正係数)
- 補正強度はマーク済みピクセルとの距離に反比例
```

**参考**: [Deconstructing Despill Algorithms](https://benmcewan.com/blog/understanding-despill-algorithms)

...
#### G. 背景推定の中央値化 ✅ ループ4で実装済み

FindBackground/FindBackgroundMaskの両方を平均→中央値に変更。16サンプル収集→ソート→中央値。
...
#### A. OKLab色空間オプション ⏭️ ループ5でスキップ（3アルゴリズム既存、4つ目は使いにくい）
...
#### F2. 段階的デスピル ✅ ループ5で実装済み

固定強度デスピル→距離ベースの段階的デスピルに改善。マンハッタン距離1-6で強度0.7〜0に線形減衰。
